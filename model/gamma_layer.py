...
if __name__ == "__main__":
    ...
    def read_csv(path):
        ...
        for row in reader:
            try:
                df.append(list(map(float, row)))
            except ValueError:
                pass
        return np.array(df)
    name = "sigma_profile"
    model_path = f"tests/explicit_H_{name}_ReadoutNN"
    params_path = "../tests/data/params.csv"
    sigma_profiles = read_csv(f"../{model_path}/{name}_test_pred.csv")
    sigma_profiles = torch.from_numpy(sigma_profiles)
    smiles = pd.read_csv(f"../tests/data/{name}_test.csv")["smiles"]
    t0 = time.time()
    # to_numpy = lambda tensor: tensor.detach().cpu().numpy()
    to_float = lambda tensor: float(tensor)
    results = [activ_coef_generates(smi, s, params_path) for s, smi in zip(sigma_profiles, smiles)]
    results = [list(map(to_float, res)) for res in results]
    activity_coef_df = pd.DataFrame(results)
    activity_coef_df = pd.concat([smiles, activity_coef_df], axis=1)
    print(activity_coef_df)


    t1 = time.time()
    print(t1- t0)
    # activity_coef_df.to_csv(f"../tests/data/activity_coefficients_test.csv", index=None)
    activity_coef_df.to_csv(f"../{model_path}/activity_coefficients_test_pred.csv", index=None)
